...
def estimate_cost(
    ...
    """
    Call simulate_inventory_core once and return a single cost (average cost per period).
    """
...
def sample_params(n_samples, seed=None):
    """
    Uniform sampling with replacement over all feasible (s, S, mu) combinations.
    """
    if seed is not None:
        random.seed(seed)
...
def generate_dataset(n_samples=10000, seed=0):
    """
    Generate n_samples samples with columns:
        [mu, s+S, S-s, 1/(S-s), cost]
    Return a numpy array of shape (n_samples, 5).
    """
    # Uniformly sample (s, S, mu) from the parameter space
    param_samples = sample_params(n_samples, seed=seed)

    # For reproducibility, use a numpy RNG to generate a simulation seed for each sample
    rng = np.random.default_rng(seed + 12345)

    # Columns: mu, sum_sS, gap, inv_gap, cost
    data = np.empty((n_samples, 5), dtype=float)
