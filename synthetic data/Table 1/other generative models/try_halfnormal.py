...
class Dataset:
    def __init__(self, args):
        ...
        # The network is fed with standardized x/y
...
# x_test in the original scale, used for standardization with each dataset's x_mean/x_std
...
for i in range(runi):
    ...
    ks, _ = stats.kstest(y_pred_flat, 'halfnorm', args=(loc, scale))
    wd = stats.wasserstein_distance(
        y_pred_flat,
        stats.halfnorm.rvs(loc=loc, scale=scale, size=k)
    )
