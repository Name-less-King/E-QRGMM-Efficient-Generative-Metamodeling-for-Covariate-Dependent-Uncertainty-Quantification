...
class Dataset:
    ...
    def bootstrap(self):
        """Bootstrap resampling, returns a new Dataset instance with resampled data."""
        idx = np.random.choice(len(self.x), size=len(self.x), replace=True)
        x_boot = self.x[idx]
        y_boot = self.y[idx]
        # Rebuild Dataset using bootstrapped data in the original scale (standardization is redone internally)
...
def boot_once(x, y, k, args):
    ...
    - data_boot internally standardizes X and Y;
    - x_test is standardized using data_boot's x_mean/x_std;
    - generated y is transformed back using data_boot's y_mean/y_std.
    """
    # First construct a Dataset from (x, y), then perform one bootstrap on it
    tmp = Dataset(args, x=x, y=y)
    data_boot = tmp.bootstrap()  # data_boot has its own x_mean/x_std, y_mean/y_std
...
x0_2 = -1.0
x0_3 = 3.0
# x_test in the original scale; actual standardization is done inside each data_boot
x_test_raw = np.array([x0_1, x0_2, x0_3], dtype=np.float32)

args = get_args('halfnormal')
args = modify_args(args)

loc = a0 + a1 * x0_1 + a2 * x0_2 + a3 * x0_3
scale = r0 + r1 * x0_1 + r2 * x0_2 + r3 * x0_3

# Theoretical mean and quantile of the half-normal distribution
mean_gt = stats.halfnorm.stats(loc=loc, scale=scale, moments='m')
mean_cover_flag = 0

quantile_gt = stats.halfnorm.ppf(0.8, loc=loc, scale=scale)
