...
class Dataset:
    def __init__(self, args, x=None, y=None):
        ...
        # ------ What is fed into the model is standardized x / y ------
    ...
    def bootstrap(self):
        """Bootstrap resampling, returns a new Dataset instance with resampled data in the original scale."""
        idx = np.random.choice(len(self.x), size=len(self.x), replace=True)
        x_boot = self.x[idx]
        y_boot = self.y[idx]
        # Construct a new Dataset using (x_boot, y_boot) in the original scale; standardization will be redone inside
...
@ray.remote
def boot_once_remote(x, y, k, args):
    return boot_once(x, y, k, args)


def boot_once(x, y, k, args):
    """
    Perform one bootstrap on a batch of (x, y):
    - first resample in the original scale;
    - obtain data_boot (which standardizes X and Y internally);
    - standardize x_test using data_boot's x_mean/x_std;
    - train the rectified model in the standardized space, then transform predictions back;
    - compute mean / 80% quantile / tail probability.
    """
    # First construct a Dataset, then perform one bootstrap on it
    tmp = Dataset(args, x=x, y=y)
    data_boot = tmp.bootstrap()  # data_boot already has x_mean / x_std / y_mean / y_std

    # Standardize x_test using this bootstrap dataset's X mean/std
    ...
    # Train rectified flow on standardized data
...
x0_2 = -1.0
x0_3 = 3.0
# x_test in the original scale; there is only this one globally,
# and the actual standardization is done inside each data_boot
x_test_raw = np.array([x0_1, x0_2, x0_3], dtype=np.float32)

args = get_args('normal')
...
quantile_gt = mean_gt + stats.norm.ppf(0.8) * std_gt
